...
- Montants : une entrée s’affiche en bleu glacier précédée de « + » ; une dépense reste en blanc doux (`--text`). Pas de vert ni de rouge sur les montants. `--negative` est réservé aux erreurs et aux retards, `--warning` aux avertissements. Jamais la couleur seule : un signe, une flèche ou un libellé porte toujours le sens.
