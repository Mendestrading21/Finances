...
- Montants (choix de l’utilisateur, 24 septembre 2026) : les entrées en vert adouci `--positive` `#8FD1B5` (précédées de « + »), les sorties en rouge adouci `--negative` `#E8A3AE`, sur les lignes d’opérations et d’abonnements, leurs flèches, les indicateurs d’entrées/sorties et les barres Entrées/Dépenses ; les virements et les totaux mixtes restent neutres. Ce sont les deux seules couleurs d’état admises à côté de l’accent bleu glacier, jamais en aplat ni en halo. Jamais la couleur seule : un signe, une flèche ou un libellé porte toujours le sens.
